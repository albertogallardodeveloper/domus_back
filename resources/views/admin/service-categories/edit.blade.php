...
<form action="{{ route('admin.service-categories.update', $serviceCategory) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow p-6 max-w-lg">
    @csrf @method('PUT')
    <div class="mb-4">
        <label class="block font-semibold mb-1">Nombre</label>
        <input type="text" name="name" class="w-full border rounded p-2" value="{{ old('name', $serviceCategory->name) }}" required>
    ...
    <div class="mb-4">
        ...
        @if($serviceCategory->image_url)
            <img src="{{ $serviceCategory->image_url }}" alt="" class="w-24 h-24 rounded shadow mb-2">
        @endif
