    ...
    <table class="min-w-full">
        ...
        @forelse($categories as $category)
            <tr class="border-b hover:bg-blue-50">
                ...
                <td class="px-3 py-2">
                    @if($category->parent)
                        <span class="bg-gray-100 text-gray-700 text-xs px-2 py-1 rounded">{{ $category->parent->name }}</span>
                    @else
                ...
                <td class="px-3 py-2">
                    @if($category->image_url)
                        <img src="{{ $category->image_url }}" alt="" class="w-12 h-12 rounded shadow">
                    @else
        ...
        @empty
            <tr>
                <td colspan="5" class="px-3 py-6 text-center text-gray-500">No hay categorías registradas.</td>
            </tr>
        @endforelse
        </tbody>
    </table>
