    ...
    <table class="min-w-full">
        ...
        @forelse($faqs as $faq)
            <tr class="border-b hover:bg-blue-50">
                ...
                <td class="px-3 py-2 font-semibold">{{ $faq->category }}</td>
                <td class="px-3 py-2">{{ $faq->title }}</td>
                <td class="px-3 py-2">
                    @if($faq->status === 'Uploaded')
                        <span class="inline-block bg-green-100 text-green-800 text-xs px-2 py-0.5 rounded">Subida</span>
                    @else
                        <span class="inline-block bg-yellow-100 text-yellow-800 text-xs px-2 py-0.5 rounded">Pendiente</span>
                    @endif
                </td>
                <td class="px-3 py-2">
                    <div class="flex gap-2">
                        ...
                        <form action="{{ route('admin.faqs.destroy', $faq) }}" method="POST"
                              onsubmit="return confirm('¿Eliminar FAQ?')" class="inline">
                            @csrf @method('DELETE')
                            <button type="submit"
        ...
        @empty
            <tr>
                <td colspan="5" class="px-3 py-6 text-center text-gray-500">No hay FAQs registradas.</td>
            </tr>
        @endforelse
        </tbody>
    </table>
