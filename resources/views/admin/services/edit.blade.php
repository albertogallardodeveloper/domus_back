...
<form action="{{ route('admin.services.update', $service) }}" method="POST" class="bg-white rounded-xl shadow p-6 max-w-lg">
    @csrf @method('PUT')
    <div class="mb-4">
    ...
    <div class="mb-4">
        ...
        <select name="user_app_id" class="w-full border rounded p-2" required>
            ...
            @foreach($professionals as $pro)
                <option value="{{ $pro->id }}" {{ old('user_app_id', $service->user_app_id) == $pro->id ? 'selected' : '' }}>
    ...
    <div class="mb-4">
        ...
        <input type="number" name="price" step="0.01" min="0" class="w-full border rounded p-2" value="{{ old('price', $service->price) }}">
        @error('price') <div class="text-red-600 text-sm">{{ $message }}</div> @enderror
    </div>
    <div class="mb-4">
        <label class="block font-semibold mb-1">Descripción</label>
        <textarea name="description" rows="3" class="w-full border rounded p-2">{{ old('description', $service->description) }}</textarea>
