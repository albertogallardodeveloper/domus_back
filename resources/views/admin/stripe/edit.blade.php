...
<h1 class="text-2xl font-bold flex items-center gap-2 mb-6">
    <i data-feather="percent"></i> Editar código promocional
</h1>
...
<form action="{{ route('admin.promo-codes.update', $promo) }}" method="POST"
    ...
    <div>
        ...
        <input name="expires_at" type="date"
               value="{{ old('expires_at', $promo->expires_at ? \Carbon\Carbon::parse($promo->expires_at)->format('Y-m-d') : '') }}"
    ...
    <div>
        ...
        <input name="max_redemptions" type="number" min="1"
               value="{{ old('max_redemptions', $promo->max_redemptions) }}"
               class="form-input w-full border-gray-300 rounded px-3 py-2">
